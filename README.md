...
## Exceptions
Apart from the built-in match exceptions, there is also the hard variant of those which is unhandleable (except using wildcard) caused when the exception caused is outside of the lexical scope of the calling site e.g. if there is a match exception within a function that is passed as a parameter.  This also happens if the name of the exception isn't known in scope, in which case it's not possible to handle it by name.

Named exceptions can be declared using <code>!&#96;exception&#95;name</code> and if desired the scope for throwing them can be limited by using <code>@&#96;exception&#95;name</code> (but you can still catch).  To catch a named exception you use the <code>&#96;exception&#95;name</code> operator, normally written prefix as <code>&#96;exception&#95;name[]</code>.  To throw a named exception you do <code>#&#96;exception&#95;name</code> and you can raise the match exception and general exception explicitly with <code>#&#96;</code> and <code>#&#96;&#95;</code>.  <code>&#96;&#95;[]</code> is of course a general exception handler which will handle any exception.
